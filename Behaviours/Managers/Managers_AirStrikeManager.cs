...
public class AirStrikeManager : MonoBehaviour
{
    ...
    public void Awake()
    {
        SandboxSpawnRegistry.Register("Airstrike", (point, normal, args) => SpawnAirstrike(point));
    }
    ...
    public void Update()
    {
        ...
        if (Cursor != null)
        {
            ...
            if (primaryDown)
            {
                if (canPlace)
                {
                    SpawnAirstrike(hitInfo.point);
                    WeaponNet.BroadcastProjectile("Airstrike", hitInfo.point, Vector3.up, null);
