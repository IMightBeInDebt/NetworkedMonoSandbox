...
public class WeaponManager : MonoBehaviour, IOnEventCallback
{
    ...
    private void Awake()
    {
        SandboxSpawnRegistry.Register("Melon", (point, normal, args) => SpawnMelon(point, normal));
    }
            ...
            if (canFire)
            {
                if (Time.time > lastShot + 0.5f)
                {
                    if (holdingWeapon == 0)
                    {
                        ...
                        Destroy(hitPoint, 3);
                        WeaponNet.BroadcastShot(holdingWeapon, hit.point, hit.normal);
                    ...
                    if (holdingWeapon == 6)
                    {
                        ...
                        {
                            Color paintColor = colourGradient.Evaluate(colourTimestamp);
                            hit.transform.GetComponent<Renderer>().material.color = paintColor;

                            if (SandboxObjectRegistry.TryGetId(hit.transform, out string netId))
                            {
                                WeaponNet.BroadcastColorize(netId, paintColor);
                            }
                        }
                ...
                if (Time.time > lastShot + 1.25f)
                {
                    if (holdingWeapon == 2)
                    {
                        ...
                        Vector3 spawnPos = HeldWeapon.transform.GetChild(1).position;
                        Vector3 launchDir = rightHand.transform.up;
                        SpawnMelon(spawnPos, launchDir);
                        WeaponNet.BroadcastProjectile("Melon", spawnPos, launchDir, null);
    ...
    private GameObject SpawnMelon(Vector3 position, Vector3 upDirection)
    {
        GameObject melon = Instantiate(MelonModel);
        melon.transform.position = position;
        melon.transform.GetComponent<Rigidbody>().AddForce(upDirection * 2500f);
        melon.transform.GetComponent<Rigidbody>().angularVelocity = upDirection * 100f;
    ...
    {
        if (!WeaponNet.TryDecodeShot(photonEvent.Code, photonEvent.CustomData, out int weaponIndex, out Vector3 hitPoint, out Vector3 hitNormal)) return;

        Player sender = PhotonNetwork.CurrentRoom?.GetPlayer(photonEvent.Sender);
        if (!SandboxNet.PlayerHasSandbox(sender)) return;

        PlayRemoteImpact(weaponIndex, hitPoint, hitNormal);
    }

    private void PlayRemoteImpact(int weaponIndex, Vector3 hitPoint, Vector3 hitNormal)
    {
        if (weaponIndex == 4)
        {
            GameObject explodeObj = Instantiate(LaserExplode);
            explodeObj.transform.position = hitPoint;
            explodeObj.transform.localScale = new Vector3(0.15f, 0.15f, 0.15f);
            Destroy(explodeObj, 3);

            Collider[] colliders = Physics.OverlapSphere(hitPoint, 24f);
            foreach (Collider nearby in colliders)
            {
                Rigidbody rig = nearby.GetComponent<Rigidbody>();
                rig?.AddExplosionForce(2500f * 4f, hitPoint, 8f);
            }
            foreach (Collider nearby in colliders)
            {
            ...
            playerRb.AddExplosionForce(2500f * 4f * Mathf.Sqrt(playerRb.mass), hitPoint, 5 + (0.75f * 4f));
            return;
        }
        
        GameObject hitPointVfx = Instantiate(HitPointParticle);
        hitPointVfx.transform.position = hitPoint;
        hitPointVfx.transform.forward = hitNormal;
        hitPointVfx.transform.localScale = (weaponIndex == 0 || weaponIndex == 5) ? new Vector3(0.6f, 0.6f, 0.6f) : new Vector3(0.8f, 0.8f, 0.8f);
        Destroy(hitPointVfx, 3);

        float force = weaponIndex switch
        {
            0 => 600f, // Revolver
            5 => 650f, // Banana Gun
            1 => 900f, // Shotgun/Rifle
            7 => 900f, // Assault Rifle
            3 => 1000f, // Sniper
            _ => 700f,
        };

        Collider[] nearbyColliders = Physics.OverlapSphere(hitPoint, 0.6f);
        Collider closest = null;
        float closestDist = float.MaxValue;
        foreach (Collider c in nearbyColliders)
        {
            if (c.attachedRigidbody == null) continue;
            float d = Vector3.Distance(c.transform.position, hitPoint);
            if (d < closestDist) { closestDist = d; closest = c; }
        }

        if (closest != null)
        {
            closest.attachedRigidbody.AddExplosionForce(force * weaponMult, hitPoint, 3f);
            closest.GetComponent<BombDetonate>()?.Explode();
            closest.GetComponent<Explode>()?.ExplodeObject();
        }
    }
}
